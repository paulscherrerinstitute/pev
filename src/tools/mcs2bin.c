/*=========================< begin file & file header >=======================
 *  References
 *  
 *    filename : mcs2bin.c
 *    author   : JFG
 *    company  : IOxOS
 *    creation : june 30,2008
 *    version  : 0.0.1
 *
 *----------------------------------------------------------------------------
 *  Description
 *
 *    This program allows to convert an ASCII "mcs" file in its binary form
 *    ready to be loaded in the PEV110 SFLASH, using the XprsMon command
 *    -> sflash load <offset> <filename>
 *
 *----------------------------------------------------------------------------
 *  Copyright Notice
 *  
 *    Copyright and all other rights in this document are reserved by 
 *    IOxOS Technologies SA. This documents contains proprietary information    
 *    and is supplied on express condition that it may not be disclosed, 
 *    reproduced in whole or in part, or used for any other purpose other
 *    than that for which it is supplies, without the written consent of  
 *    IOxOS Technologies SA                                                        
 *
 *----------------------------------------------------------------------------
 *  Change History
 *  
 * $Log: mcs2bin.c,v $
 * Revision 1.6  2012/06/29 08:47:01  kalantari
 * checked in the PEV_4_14 got from JF ioxos
 *
 * Revision 1.4  2012/03/15 09:30:15  ioxos
 * bug if bitstream > 0x800000 (unsigned charpwd) [JFG]
 *
 * Revision 1.3  2011/03/14 14:51:08  ioxos
 * support files up to 16 MBytes [JFG]
 *
 * Revision 1.2  2008/11/12 14:22:46  ioxos
 * add support for a pex instruction [JFG]
 *
 * Revision 1.1  2008/07/18 14:59:29  ioxos
 * file creation [JFG]
 *
 *
 *=============================< end file header >============================*/

#ifndef lint
static char *rcsid = "$Id: mcs2bin.c,v 1.6 2012/06/29 08:47:01 kalantari Exp $";
#endif

#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <strings.h>
#include <sys/types.h>

char line[256];
unsigned char ins[8];

int
swap_32( int data)
{
  char ci[4];
  char co[4];

  *(int *)ci = data;
  co[0] = ci[3];
  co[1] = ci[2];
  co[2] = ci[1];
  co[3] = ci[0];

  return( *(int *)co);
}

unsigned char data[256];
unsigned char *bitstream;

main( int argc,
      void *argv[])
{
  char *infilename, *outfilename;
  FILE *infile, *outfile;
  int i, n;
  int type, len;
  unsigned int addr, addr_lo, addr_hi;
  unsigned char cks;

  i = 1;
  infilename = (char *)0;
  outfilename = (char *)0;
  while( i < argc)
  {
    char *p;

    p = argv[i++];
    printf("%s\n", p);
    if( p[0] == '-')
    {
      switch( p[1])
      {
        case 'o':
        {
	  p = argv[i++];
	  outfilename = p;
	  printf("%s\n", p);
	  break;
	}
        default:
        {
	  printf("bad option\n");
	  exit(0);
	}
      }
    }
    else
    {
      infilename = p;
      break;
    }
  }
  if( !infilename)
  {
    printf("usage: mcs2bin -o outfile infile");
    exit(0);
  }
  infile = fopen( infilename, "r");
  if( !infile)
  {
    printf("cannot open input file %s\n", infilename);
    exit(0);
  }
  if( !outfilename)
  {
    outfilename = malloc( strlen(infilename) + 4);
    strcpy( outfilename, infilename);
    strcat( outfilename,".sfl");
  }
  outfile = fopen( outfilename, "w");
  if( !outfile)
  {
    printf("cannot open output file %s\n", outfilename);
    fclose( infile);
    exit(0);
  }
  printf("NEW : converting file %s -> %s\n", infilename, outfilename);
  n = 0;
  addr_hi = 0;
  addr_lo = 0;
  bitstream = (char *)malloc( 0x1000000);
  if( !bitstream)
  {
    printf("cannot allocate buffer for bistream file\n");
    fclose( outfile);
    fclose( infile);
    exit( 0);
  }
  while( fgets( line, 256, infile))
  {
    if( line[0] == ':')
    {
      sscanf( line, ":%2x%4x%2x", &len, &addr_lo, &type);
      cks = len + type + (addr_lo & 0xff) + (( addr_lo >> 8) & 0xff);
      for( i = 0; i <= len; i++)
      {
	sscanf( &line[9+(2*i)], "%2x", &data[i]);
	cks += data[i];
      }
      if( cks)
      {
	printf("bad checksum at line %d\n", n);
	goto bin2srec_error;
      }
      switch( type)
      {
        case 0:
	{
	  addr = addr_hi + addr_lo;
	  if( addr > (0x1000000 - len))
	  {
	    printf("address to big [%08x] at line %d\n", addr, n);
	    goto bin2srec_error;
	  }
	  for( i = 0; i < len; i++)
	  {
	    bitstream[addr++] = data[i];
	  }
	  break;
	}
        case 1:
	{
	  break;
	}
        case 2:
        case 4:
	{
	  if( addr_lo)
	  {
	    printf("format error at line %d\n", n);
	    goto bin2srec_error;
	  }
	  if( type == 4)
	  {
	    addr_hi = ( data[0] << 24) | ( data[1] << 16);
	  }
	  else
	  {
	    addr_hi = ( data[0] << 12) | ( data[1] << 4);
	  }
	  break;
	}
        default:
	{
	  printf("unknown record type [%2x] at line %d\n", type, n);
	  goto bin2srec_error;
	}
      }
    }
      n++;
  }
  printf("writing 0x%08x byte to FPGA bitstream file %s\n", addr, outfilename);
  fwrite( bitstream, addr, 1, outfile);
bin2srec_error:
  free( bitstream);
  fclose( outfile);
  fclose( infile);
  exit( 0);
}
